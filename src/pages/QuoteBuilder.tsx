...
import type { QuoteLineItem, CatalogItem, CatalogCategory } from '../types'
...
export default function QuoteBuilder() {
  ...
  return (
    <div className="min-h-screen bg-surface pb-48">
      {/* ── Top bar ── */}
      <div className="bg-navy text-white px-5 pt-10 pb-4 sticky top-0 z-30">
        <div className="flex items-center justify-between max-w-lg mx-auto">
          <button onClick={() => navigate(-1)} className="p-2 -mr-2 touch-feedback">
            <ArrowRight size={20} />
          </button>
          <h1 className="font-bold text-base">
            {quote?.quoteNumber ? `הצעה ${quote.quoteNumber}` : 'בונה הצעה'}
          </h1>
          <button
            onClick={handleSave}
            className="text-xs font-medium bg-white/15 px-3 py-1.5 rounded-lg touch-feedback"
          >
            שמור
          </button>
        </div>
